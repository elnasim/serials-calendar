<template>
  <div class="p-1" @click="showPopup">
    ...
    >
      ...
      >
        <div v-for="movie of dayData.content" :key="movie._id" class="h-full">
          <div
            class="w-full h-full bg-cover bg-repeat flex items-end p-1 relative mb-0.5 last:mb-0"
            :style="`background-image: url(${VITE_CDN_URL}/movies/${movie._id}.jpg);`"
          >
            <div
    ...
    <Popup v-model="isShowPopup">
      <CalendarMovieCellPopup :dayData="dayData" />
    </Popup>
  </div>
</template>
...
import type { TDayMovies } from "@/modules/calendar/types";
import Popup from "@/modules/common/components/Popup.vue";
import CalendarMovieCellPopup from "@/modules/calendar/components/CalendarMovieCellPopup.vue";
